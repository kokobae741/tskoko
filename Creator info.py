...
def log(value, status='CREATOR INFO', color='[P1]'):
...
def log_replace(value, status='CREATOR INFO', color='[G1]'):
    ...
    def start(self):
        try:
            ...
            if len(frontend_domains) == 0:
                ...
                return
            self.log('SUBCRIBE YA {} port {}'.format(self.inject_host, self.inject_port))
            while True:
                socket_client, _ = socket_server.accept()
                socket_client.recv(4096)
                domain_fronting(socket_client, frontend_domains).start()
        except Exception as exception:
            self.log('TELKOMSEL OPOK 2019 {} port {}'.format(self.inject_host, self.inject_port), color='[R1]')
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='CREATOR INFO', color='[G1]'):
    ...
    def run(self):
        try:
            ...
            self.log('CREATOR INFO !!! {} port {}'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('SUBCRIBE & Like !!! [Y1]({} port {})'.format(self.proxy_host, self.proxy_port), color='[Y1]')
        except OSError:
...
def main():
    print(colors('\n'.join([
        ...
        '[G1] SEKILAS INFO :', '[CC]',
        '[CC]*JANGAN DI SKIP VIDEO NYA*', '[CC]',
        '[CC]*SUBSCRIBE & LIKE*', '[CC]',
        '[Y1]*SCRIPT VIP*', '[CC]',
        '[G1]== SELAMAT HARI RAYA IDUL FITRI', '[CC]',
        '[G1] MOHON MAAF LAHIR DAN BATIN ==', '[CC]',
        '[Y1]*CREATOR INFO','[CC]'
    ])))
