...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/Sakti.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Sakti.txt', color='[R1]')
                return
            self.log(' subscribe biar Tamvan'.format(self.inject_host, self.inject_port))
            while True:
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='Attack', color='[P1]'):
    ...
    def run(self):
        try:
            ...
            self.log('Proses Hack Sedang Berjalan..')
            ...
            self.log('Selamat Proses Hack Sukses !'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
...
def main():
    print(colors('\n'.join([
        '[G1][Subscribe Youtube Chanel:RendiSakti][********Telkomsel*****Attack******Tolls****************Hack*****Internet*****Gratis*****2019****Via*Psipon*Pro*****Qpython****Dan****Termux]',
        '[Y1]SELAMAT DATANG MAS TAMVAN', '[R1]'
    ])))
