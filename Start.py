...
def log_replace(value, status='Proses', color='[R1]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/MrExell.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('MrExell.txt', color='[R1]')
                return
            self.log('BangKoko By.mr.Exel. kusus nggo sing ora gableg kuota ragableg pulsa. GUNAKAN DENGAN BIJAK'.format(self.inject_host, self.inject_port))
            while True:
                socket_client, _ = socket_server.accept()
                socket_client.recv(65535)
                domain_fronting(socket_client, frontend_domains).start()
        except Exception as exception:
            self.log('Kesalahan.. Silahkan Hubungi Mr.Exel !'.format(self.inject_host, self.inject_port), color='[R1]')
...
class domain_fronting(threading.Thread):
    ...
    def log(self, value, status='Proses', color='[P1]'):
    ...
    def run(self):
        try:
            ...
            self.log('Connect !'.format(self.proxy_host, self.proxy_port), color='[CC]')
        except OSError:
            self.log('ENTE KIH DURUNG ADUS YA WA', color='[R1]')
        except TimeoutError:
            self.log('{} not responding'.format(self.proxy_host), color='[R1]')

def main():
    print(colors('\n'.join([
        '[G1]BangKoko By.Mr.Exell',
        '[Y1]End_Script', '[R1]'
    ])))
