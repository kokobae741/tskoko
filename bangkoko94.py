...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/bangkoko.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check bangkoko.txt', color='[Y1]')
                return
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('kODIKIT WA TEK SERUM DIPIT, OK WA LANJUT'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('sAMPEYAN E DURUNG ADUS YA WAAAAAA ''[R2]'.format(self.proxy_host, self.proxy_port), color='[R1]')
        except OSError:
...
print G + 'Allo waaaaaaaa!!!'
print G + 'Ngenet gratis telekampret94 kusus sing ragableg kuota ragableg pulsa T E R L A L U'
def main():
    print ' [!] GUNAKAN DENGAN BIJAK YA WAAAA\n'
