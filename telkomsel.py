...
def log(value, status='start running', color='[R1]'):
...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/domain.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check domain.txt', color='[Y1]')
                return
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('server reading'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('server failed''[R2]'.format(self.proxy_host, self.proxy_port), color='[R1]')
        except OSError:
            self.log('good', color='[R1]')
        except TimeoutError:
            self.log('{} spirit'.format(self.proxy_host), color='[R1]')

G =phreaker=("""[R2]
 =======================================
    -> WELCOME TO DOMAIN FRONTING
    -> Rabbit version
    -> please login first!
 =======================================
""")
def main():
    print(colors('\n'.join([
    (phreaker),
        '[T]Domain Fronting [Rabbit Version 1.3.190427]',
        '[T](c) 2019 Aztec Rabbit' '[CC]'
    ])))
